...
TEST(ServerF04, PduLengthIncorrect_Reply03Return0)
{
    ...
    auto read_input_regs = [](uint16_t*, uint16_t, uint16_t) -> int16_t {
        return 0;
    };
...
TEST(ServerF04, ValidRequest_CallbackReturnsLessThanQuantity_NoReplyReturnEAGAIN)
{
    ...
    auto read_input_regs = [](uint16_t*, uint16_t length, uint16_t) -> int16_t {
        cb_reads++;
        if (cb_reads == 1)
        {
            return 0;
...
TEST(ServerF04, ValidRequest_WritePduReturnsLessThanLength_ReturnEAGAIN)
{
    ...
    auto read_input_regs = [](uint16_t* buffer, uint16_t length, uint16_t) -> int16_t {
        buffer[0] = 0x0100;
        buffer[1] = 0x0302;
        cb_reads++;
        return length;
